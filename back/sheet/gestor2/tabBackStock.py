# ./back/sheet/gestor/tabBackStock.py
...
except Exception:  # fallback mínimo para desarrollo sin APIs
...
class StockBackend:
    """
    Backend que usa las APIs de Sheets y expone helpers que consume el front:
      ...
      - safe_int
      - soporte opcional de 'bus' para eventos cross-tabs
    """

    def __init__(self, page, bus: Optional[object] = None):
        self.page = page
        self.bus = bus  # ⬅️ ahora soporta bus opcional
        self.sheet_id = (
    ...
    def refresh_products(self):
        if not self.api_prod:
            ...
            self.prod_by_recid = {}
            return
        self.productos = self.api_prod.list() or []      # <- normaliza None a []
    ...
    def refresh_depositos(self):
        if not self.api_depo:
            ...
            self.depo_by_recid = {}
            return
        self.depositos = self.api_depo.list() or []      # <- normaliza None a []
    ...
    def refresh_stock(self):
        if not self.api_stock:
            self.stock_rows = []
            return
        self.stock_rows = self.api_stock.list() or []    # <- normaliza None a []
    ...
    def _publish(self, topic: str, payload: Optional[dict] = None):
        if not self.bus:
            return
        try:
            # Se espera que el bus tenga un método publish(topic, payload)
